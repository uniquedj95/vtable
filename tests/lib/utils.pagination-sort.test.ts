...
import type { PaginationInterface, SortQueryInterface, TableColumnInterface } from '../../src/lib/types';
...
describe('buildPaginationInfo', () => {
  ...
  it('should show correct range for first page', () => {
    ...
    expect(buildPaginationInfo(paginator, 25)).toBe('Showing 1 to 10 of 25 entries');
  });
  ...
  it('should handle last page with fewer items', () => {
    ...
    expect(buildPaginationInfo(paginator, 25)).toBe('Showing 21 to 25 of 25 entries');
  });
});
...
describe('sortRows', () => {
  ...
  const ageColumn: TableColumnInterface = {
    label: 'Age',
    path: 'age',
    sortable: true
  };
  ...
  it('should use preSort function when provided', () => {
    ...
    const query: SortQueryInterface[] = [{ column: customColumn, order: 'asc' }];
    const sorted = sortRows(rows, query);
...
describe('initializeSortQueries', () => {
  ...
  it('should create sort queries for columns with initialSort', () => {
    const columns: TableColumnInterface[] = [
      { label: 'Name', path: 'name', initialSort: true, initialSortOrder: 'asc' },
      { label: 'Age', path: 'age' }
    ];
...
describe('updateSortQueries', () => {
  ...
  it('should toggle order when column already exists in queries', () => {
    const queries: SortQueryInterface[] = [{ column: nameColumn, order: 'asc' }];
